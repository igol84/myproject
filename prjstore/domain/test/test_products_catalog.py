...
class Test_ProductCatalog(TestProductCatalog):
    def test_01_initial(self):
        assert str([pr.name for pr in self.pc.products.values()]) == "['item1', 'item23', 'item24', 'item5', 'item2']"

    def test_02_1_unset_product_by_pr_id(self):
        self.pc.unset_product_by_pr_id('3')
        assert str([pr.name for pr in self.pc.products.values()]) == "['item1', 'item23', 'item5', 'item2']"
    ...
    def test_03_get_product_by_id(self):
        assert str(self.pc.products['2'].name) == 'item23'
        assert str(self.pc.get_product_by_id('3').name) == 'item24'

    def test_04_search(self):
        pr_list = self.pc.search(name='Item2')
        assert str([product.name for product in pr_list.values()]) == "['item23', 'item24', 'item2']"
